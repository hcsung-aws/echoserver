...
bool on_recv_echo_fake(sessionmanager<_T>& manager, _T* recv_session, packetheader* header)
{
...
public:
	fakesession(tcp::socket socket, packethandler<fakesession>* handler, std::function<void(int)> disconnect_handler)
...
protected:
	...
	size_t recv_size_ = 0;
	packethandler<fakesession>* handler_;
	int	index_ = 0;
	std::function<void(int)> disconnect_handler_;
};

class fakeserver : public tcpserver<fakesession>
{
public:
	fakeserver(boost::asio::io_context& io_context, tcp::endpoint& endpoint)
		: tcpserver(io_context, endpoint)
	{}
	...
	void do_accept() override
	{
		...
		std::shared_ptr<fakesession> created_session = std::make_shared<fakesession>(tcp::socket(io_context), &handler_, disconn_handler_dummy);
		manager_.add_session(created_session);
	}

	packethandler<fakesession>& get_packethandler() { return handler_; }
};

void parsepacket_test(std::shared_ptr<fakesession> session_recved, packethandler<fakesession>& handler, char* recv_buffer, size_t length)
{
...
{
protected:
	sessionmanager<fakesession>	manager_;
	packethandler<fakesession>	handler_;
...
protected:
	HandlerTest()
		: handler_(manager_)
		, server_(io_context_, tcp::endpoint(tcp::v4(), std::atoi("127.0.0.1")))
	{
	...
	virtual void SetUp()
	{
		server_.add_handler(packettype::echo_type, on_recv_echo_fake<fakesession>);
		server_.do_accept();
...
TEST_F(HandlerTest, recv_echo_function)
{
	...
	created_session_->do_read(std::bind(&fakeserver::parsepacket/*parsepacket_test*/, &server_, created_session_, std::placeholders::_1, std::placeholders::_2));
